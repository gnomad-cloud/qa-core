...
class Converters {
    ...
    static load_feature(filename, done) {
        let type = vars_1.Vars.suffix(filename, ".");
        if (!type)
            done("invalid file", null);
        if (type == "feature") {
            let fp = new parsers_1.FeatureParser();
            let raw = fs.readFileSync(filename).toString();
            fp.parse(raw, (feature) => {
                done(null, feature);
            });
        }
        else
            this.json_or_yaml(filename, done);
    }

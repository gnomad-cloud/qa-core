 ...
 *
 */
class FilesDialect extends Dialect_1.Dialect {
    constructor(engine) {
        super(engine);
        ...
        this.define(["I load $varname as $format from $file", "I read $varname as $format from $file"], function (name, format, filename, done) {
            let file = files_1.Files.root(this.paths, filename);
            assert(files_1.Files.exists(file), format + " file not found: " + file);
            // let converter = converts[format];
            // assert(converter, format + " files are not supported: " + file);
            format = format.toLowerCase();
            let raw = files_1.Files.load(file);
            assert(raw, format + " file is empty: " + file);
            converters_1.Converters.load(raw, format, (_err, contents) => {
                engine.debug("%s converted from: %s", file, format);
                vars_1.Vars.set(engine.vars, name, contents);
                done();
            });
        }, new Dialect_1.DialectDocs("files.load", "Load contents of file into a variable"));
        this.define(["I mkdir $folder"], function (folder, done) {
            let path = files_1.Files.root(this.paths, folder);
            mkdirp.sync(path);
            done();
        }, new Dialect_1.DialectDocs("files.dir", "Create a directory/folder"));
    }
    scope(_scope) {
        _.defaults(_scope, {
            paths: {
...
exports.FilesDialect = FilesDialect;
